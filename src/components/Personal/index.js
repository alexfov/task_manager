...
function sorting(сoef, setCoef, property, data, setData) {
    const newData = [...data];
    const sortingFn = (a, b) =>
        a[property] > b[property] ? 1 * сoef : -1 * сoef;
        ...
        (empolyee, i) =>
            empolyee.isFree && (
                ...
                >
                    ...
                    <div className="personal__group block__sub-element">
                        {empolyee.group}
                    </div>
    ...
    return (
        <div className="personal block" style={{ textAlign: "left" }}>
            <div className="personal-header personal__employee block__element">
                <div
                    className="personal__name block__sub-element"
                    onClick={onNameClick}
                >
                    Имя
                </div>
                <div
                    className="personal__group block__sub-element"
                    onClick={onGroupClick}
                >
                    Г
                </div>
                <div
                    className="personal__region block__sub-element"
                    onClick={onRegionClick}
                >
                    Р
                </div>
                <div
                    className="personal__adress block__sub-element"
            ...
            {isFormHidden || (
                <AddPersonalBtn data={personal} setData={setPersonal} />
            )}
        </div>
    );
}
