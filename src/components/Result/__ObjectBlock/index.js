...
function ObjectBlock({
    ...
    const object_cars = [...object.cars].map((carId) =>
        cars.find((car) => car.id === carId)
    );
    const object_personal = [...object.personal].map((userId) =>
        personal.find((users) => users.id === userId)
    );

    return (
        <div className="result__object" key={object.id}>
            <div className="result__object-name">{object.name}</div>
